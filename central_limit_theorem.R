...
for (i in 1:n_samples) {
   data_dist = runif(n_observations)
   data_sum = data_sum + data_dist
}

# Test for normality: if the p-value printed on the console 
# is under 0.05, then we know distribution is not normal
shapiro.test(data_sum)

# Let's split the plot window in two columns
par(mfrow = c(1, 2))
